...
import com.translate.Languages;
import com.translate.TranslateService;
...
public class MainController {
     ...
     */
    @Autowired
    public MainController(@Value("${messenger4j.appSecret}") final String appSecret,
        ...
        translateService.setAccountID("549");
        translateService.setSecretKey("a7b3b8dd0532181ad2ab10072055ae16");
    }
        ...
        return event -> {
            ...
            try {
                switch (messageText.toLowerCase()) {
                    ...
                    case "hi":
                        sendTextMessage(senderId, "Hi! Enter words for translation)");
                        break;

                    case "thanks":
                        sendTextMessage(senderId, "You're welcome :)");
                        break;

                    default:
                          sendReadReceipt(senderId);
                          sendTypingOn(senderId);
                          sendTranslatedText(senderId, messageText);
    ...
    private void sendTranslatedText(String recipientId, String rowText) throws Exception {

        rowText = rowText.trim();
        String languageDef  = rowText.substring(0,2);
        String translatedText;

        if(languageDef.toLowerCase().equals(Languages.en.toString()))
           translatedText = translateService.translate(rowText.substring(3,rowText.length()), Languages.en,Languages.ru);
        else
           translatedText = translateService.translate(rowText, Languages.ru,Languages.en);

        sendTextMessage(recipientId, translatedText);
